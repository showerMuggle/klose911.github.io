...
+ 如果一个类只有原始变量类型和final类型的属性时候，调用super.clone就能正确克隆
  #+BEGIN_SRC java
  ...
  #+END_SRC
+ 克隆必须保证将来不会伤害到原始的对象，并确保正确地创建被克隆对象中的约束条件。因此如果对象有可变的属性时候，只是调用super.clone是不足够的，必须逐个属性进行clone
  #+BEGIN_SRC java
    public class Stack {
  ...
  #+END_SRC
简单调用super.clone会导致克隆对象的属性elements没有被正确复制，只是和旧的elements拥有相同引用，这意味着当克隆对象调用pop，push方法，旧的对象elements数组也会被修改
#+BEGIN_SRC java
...
+ 某些情况下，简单的调用属性clone方法仍然不够，必须手动复制
  #+BEGIN_SRC java
...
[[file:class_interface.org][Next：类和接口]]　[[file:create_destroy_objects.org][Previous：创建和销毁对象]] [[file:effj.org][Up：目录]]
