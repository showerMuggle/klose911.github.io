...
第二步，发起连接操作，判断连接结果，代码如下：
  ...
  #+ATTR_HTML: image :width 70% 
  [[file:pic/netty-client-do-connect.png]]
...
第三步，由NioEventLoop的多路复用器轮询连接操作结果，代码如下：
  ...
  #+ATTR_HTML: image :width 70% 
  [[file:pic/netty-client-selector.png]]

判断连接结果：

  #+ATTR_HTML: image :width 70% 
  [[file:pic/netty-client-check-connection-result.png]]

如果或连接成功，重新设置监听位为READ：

  #+ATTR_HTML: image :width 70% 
  [[file:pic/netty-client-set-read-interest-op.png]]
